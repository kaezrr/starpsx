use starpsx_renderer::utils::Color;
use starpsx_renderer::utils::RectTextureOptions;
use starpsx_renderer::utils::Texture;
use starpsx_renderer::utils::TextureOptions;
use starpsx_renderer::vec2::Vec2;

use super::Command;
use super::GP0State;
use super::Gpu;
use super::VramCopyFields;
use super::utils::parse_clut_uv;
use super::utils::parse_page_uv;
use super::utils::parse_uv;
use super::utils::parse_xy;

#[allow(clippy::unused_self)]
impl Gpu {
    pub const fn gp0_nop(&mut self, _params: &[Command]) -> GP0State {
        // Do nothing
        GP0State::AwaitCommand
    }

    pub const fn gp0_clear_cache(&mut self, _params: &[Command]) -> GP0State {
        // unimplemented
        GP0State::AwaitCommand
    }

    pub fn gp0_draw_mode(&mut self, params: &[Command]) -> GP0State {
        let cmd = params[0];

        self.status.set_page_base_x(cmd.page_base_x());
        self.status.set_page_base_y(cmd.page_base_y());
        self.status.set_semi_transparency(cmd.semi_transparency());
        self.status.set_texture_depth(cmd.texture_depth());
        self.status.set_dithering(cmd.dithering());
        self.status.set_draw_to_display(cmd.draw_to_display());
        self.status.set_texture_disable(cmd.texture_disable());
        self.renderer.ctx.rect_texture = Texture::new(cmd.0 as u16, None);

        GP0State::AwaitCommand
    }

    pub fn gp0_drawing_area_top_left(&mut self, params: &[Command]) -> GP0State {
        self.renderer.ctx.drawing_area_top_left = {
            let x = params[0].x_coordinates();
            let y = params[0].y_coordinates();
            Vec2::new(i32::from(x), i32::from(y))
        };
        GP0State::AwaitCommand
    }

    pub fn gp0_drawing_area_bottom_right(&mut self, params: &[Command]) -> GP0State {
        self.renderer.ctx.drawing_area_bottom_right = {
            let x = params[0].x_coordinates();
            let y = params[0].y_coordinates();
            Vec2::new(i32::from(x), i32::from(y))
        };
        GP0State::AwaitCommand
    }

    pub fn gp0_drawing_area_offset(&mut self, params: &[Command]) -> GP0State {
        self.renderer.ctx.drawing_area_offset = {
            let x = (params[0].x_offset() << 5) as i16 >> 5;
            let y = (params[0].y_offset() << 5) as i16 >> 5;
            Vec2::new(i32::from(x), i32::from(y))
        };
        GP0State::AwaitCommand
    }

    pub fn gp0_texture_window(&mut self, params: &[Command]) -> GP0State {
        self.renderer.ctx.texture_window_mask = {
            let x = params[0].window_mask_x();
            let y = params[0].window_mask_y();
            Vec2::new(i32::from(x), i32::from(y))
        };

        self.renderer.ctx.texture_window_offset = {
            let x = params[0].window_offset_x();
            let y = params[0].window_offset_y();
            Vec2::new(i32::from(x), i32::from(y))
        };
        GP0State::AwaitCommand
    }

    pub fn gp0_mask_bit_setting(&mut self, params: &[Command]) -> GP0State {
        self.status
            .set_force_set_mask_bit(params[0].force_set_mask_bit());
        self.status
            .set_preserve_masked_pixels(params[0].preserve_masked_pixels());
        self.renderer.ctx.force_set_masked_bit = params[0].force_set_mask_bit();
        self.renderer.ctx.preserve_masked_pixels = params[0].preserve_masked_pixels();
        GP0State::AwaitCommand
    }

    pub fn gp0_image_store(&mut self, params: &[Command]) -> GP0State {
        let Vec2 { x, y } = parse_xy(params[1].0);
        let Vec2 {
            x: width,
            y: height,
        } = parse_xy(params[2].0);
        let vram_x = x as u16;
        let vram_y = y as u16;

        let width = match width as u16 {
            0 => 1024,
            x => x,
        };

        let height = match height as u16 {
            0 => 512,
            x => x,
        };

        GP0State::CopyFromVram(VramCopyFields {
            vram_x,
            vram_y,
            width,
            height,
            current_row: 0,
            current_col: 0,
        })
    }

    pub fn gp0_image_load(&mut self, params: &[Command]) -> GP0State {
        let Vec2 { x, y } = parse_xy(params[1].0);
        let Vec2 {
            x: width,
            y: height,
        } = parse_xy(params[2].0);

        let vram_x = x as u16;
        let vram_y = y as u16;

        let width = match width as u16 {
            0 => 1024,
            x => x,
        };

        let height = match height as u16 {
            0 => 512,
            x => x,
        };

        GP0State::CopyToVram(VramCopyFields {
            vram_x,
            vram_y,
            width,
            height,
            current_row: 0,
            current_col: 0,
        })
    }
    // DRAW COMMANDS
    pub fn gp0_quick_rect_fill(&mut self, params: &[Command]) -> GP0State {
        let color = Color::new_8bit(params[0].0);
        let v = parse_xy(params[1].0);
        let Vec2 {
            x: width,
            y: height,
        } = parse_xy(params[2].0);

        self.renderer.vram_quick_fill(v, width, height, color);
        GP0State::AwaitCommand
    }

    pub fn gp0_vram_to_vram_blit(&mut self, params: &[Command]) -> GP0State {
        let src = parse_xy(params[1].0);
        let dst = parse_xy(params[2].0);
        let size = parse_xy(params[3].0);

        self.renderer.vram_self_copy(src, dst, size);
        GP0State::AwaitCommand
    }

    pub fn gp0_poly_mono<const QUAD: bool, const SEMI_TRANS: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let color = Color::new_5bit(params[0].0);

        let v0 = parse_xy(params[1].0);
        let v1 = parse_xy(params[2].0);
        let v2 = parse_xy(params[3].0);

        self.renderer
            .draw_triangle::<SEMI_TRANS>([v0, v1, v2], color);

        if QUAD {
            let v3 = parse_xy(params[4].0);
            self.renderer
                .draw_triangle::<SEMI_TRANS>([v1, v2, v3], color);
        }

        GP0State::AwaitCommand
    }

    pub fn gp0_poly_shaded<const QUAD: bool, const SEMI_TRANS: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let v0 = parse_xy(params[1].0);
        let v1 = parse_xy(params[3].0);
        let v2 = parse_xy(params[5].0);

        let c0 = Color::new_5bit(params[0].0);
        let c1 = Color::new_5bit(params[2].0);
        let c2 = Color::new_5bit(params[4].0);

        self.renderer
            .draw_triangle_shaded::<SEMI_TRANS>([v0, v1, v2], [c0, c1, c2]);

        if QUAD {
            let v3 = parse_xy(params[7].0);
            let c3 = Color::new_5bit(params[6].0);
            self.renderer
                .draw_triangle_shaded::<SEMI_TRANS>([v1, v2, v3], [c1, c2, c3]);
        }

        GP0State::AwaitCommand
    }

    pub fn gp0_poly_texture<const QUAD: bool, const SEMI_TRANS: bool, const BLEND: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let color = Color::new_5bit(params[0].0);

        let v0 = parse_xy(params[1].0);
        let v1 = parse_xy(params[3].0);
        let v2 = parse_xy(params[5].0);

        let (clut, uv0) = parse_clut_uv(params[2].0);
        let (texture, uv1) = parse_page_uv(params[4].0, clut);
        let uv2 = parse_uv(params[6].0);

        // for some goddamn reason this also updates the global texture
        self.renderer.ctx.rect_texture = texture;

        self.renderer.draw_triangle_textured::<SEMI_TRANS, BLEND>(
            [v0, v1, v2],
            color,
            TextureOptions {
                texture,
                uvs: [uv0, uv1, uv2],
            },
        );

        if QUAD {
            let v3 = parse_xy(params[7].0);
            let uv3 = parse_uv(params[8].0);
            self.renderer.draw_triangle_textured::<SEMI_TRANS, BLEND>(
                [v1, v2, v3],
                color,
                TextureOptions {
                    texture,
                    uvs: [uv1, uv2, uv3],
                },
            );
        }

        GP0State::AwaitCommand
    }

    pub fn gp0_poly_texture_shaded<const QUAD: bool, const SEMI_TRANS: bool, const BLEND: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let c0 = Color::new_5bit(params[0].0);
        let c1 = Color::new_5bit(params[3].0);
        let c2 = Color::new_5bit(params[6].0);

        let v0 = parse_xy(params[1].0);
        let v1 = parse_xy(params[4].0);
        let v2 = parse_xy(params[7].0);

        let (clut, uv0) = parse_clut_uv(params[2].0);
        let (texture, uv1) = parse_page_uv(params[5].0, clut);
        let uv2 = parse_uv(params[8].0);

        self.renderer
            .draw_triangle_textured_shaded::<SEMI_TRANS, BLEND>(
                [v0, v1, v2],
                [c0, c1, c2],
                TextureOptions {
                    texture,
                    uvs: [uv0, uv1, uv2],
                },
            );

        if QUAD {
            let c3 = Color::new_5bit(params[9].0);
            let v3 = parse_xy(params[10].0);
            let uv3 = parse_uv(params[11].0);

            self.renderer
                .draw_triangle_textured_shaded::<SEMI_TRANS, BLEND>(
                    [v1, v2, v3],
                    [c1, c2, c3],
                    TextureOptions {
                        texture,
                        uvs: [uv1, uv2, uv3],
                    },
                );
        }

        GP0State::AwaitCommand
    }

    pub fn gp0_line_mono<const SEMI_TRANS: bool>(&mut self, params: &[Command]) -> GP0State {
        let v0 = parse_xy(params[1].0);
        let v1 = parse_xy(params[2].0);
        let color = Color::new_5bit(params[0].0);

        self.renderer.draw_line::<SEMI_TRANS>([v0, v1], color);
        GP0State::AwaitCommand
    }

    pub fn gp0_line_shaded<const SEMI_TRANS: bool>(&mut self, params: &[Command]) -> GP0State {
        let v0 = parse_xy(params[1].0);
        let v1 = parse_xy(params[3].0);

        let c0 = Color::new_5bit(params[0].0);
        let c1 = Color::new_5bit(params[2].0);

        self.renderer
            .draw_line_shaded::<SEMI_TRANS>([v0, v1], [c0, c1]);
        GP0State::AwaitCommand
    }

    pub fn gp0_line_mono_poly<const SEMI_TRANS: bool>(
        &mut self,
        vertices: Vec<u32>,
        colors: &[u32],
    ) -> GP0State {
        let vertices: Vec<Vec2> = vertices.into_iter().map(parse_xy).collect();
        let color = Color::new_5bit(colors[0]);

        for i in 1..vertices.len() {
            self.renderer
                .draw_line::<SEMI_TRANS>([vertices[i - 1], vertices[i]], color);
        }
        GP0State::AwaitCommand
    }

    pub fn gp0_line_shaded_poly<const SEMI_TRANS: bool>(
        &mut self,
        vertices: Vec<u32>,
        colors: &[u32],
    ) -> GP0State {
        let vertices: Vec<Vec2> = vertices.into_iter().map(parse_xy).collect();
        let colors: Vec<Color> = colors.iter().copied().map(Color::new_5bit).collect();

        for i in 1..vertices.len() {
            self.renderer.draw_line_shaded::<SEMI_TRANS>(
                [vertices[i - 1], vertices[i]],
                [colors[i - 1], colors[i]],
            );
        }
        GP0State::AwaitCommand
    }

    pub fn gp0_rect_fixed<const SIDE: i32, const SEMI_TRANS: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let color = Color::new_8bit(params[0].0);
        let v = parse_xy(params[1].0);
        self.renderer
            .draw_rectangle::<SEMI_TRANS>(v, Vec2::new(SIDE, SIDE), color);
        GP0State::AwaitCommand
    }

    pub fn gp0_rect_texture_fixed<const SIDE: i32, const SEMI_TRANS: bool, const BLEND: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let color = Color::new_8bit(params[0].0);
        let v = parse_xy(params[1].0);
        let (clut, uv) = parse_clut_uv(params[2].0);

        self.renderer.draw_rectangle_textured::<SEMI_TRANS, BLEND>(
            v,
            Vec2::new(SIDE, SIDE),
            color,
            &RectTextureOptions { clut, uv },
        );

        GP0State::AwaitCommand
    }

    pub fn gp0_rect_variable<const SEMI_TRANS: bool>(&mut self, params: &[Command]) -> GP0State {
        let color = Color::new_8bit(params[0].0);
        let v = parse_xy(params[1].0);
        let side = parse_xy(params[2].0);

        self.renderer.draw_rectangle::<SEMI_TRANS>(v, side, color);

        GP0State::AwaitCommand
    }

    pub fn gp0_rect_texture_variable<const SEMI_TRANS: bool, const BLEND: bool>(
        &mut self,
        params: &[Command],
    ) -> GP0State {
        let color = Color::new_8bit(params[0].0);
        let v = parse_xy(params[1].0);
        let (clut, uv) = parse_clut_uv(params[2].0);
        let side = parse_xy(params[3].0);

        self.renderer.draw_rectangle_textured::<SEMI_TRANS, BLEND>(
            v,
            side,
            color,
            &RectTextureOptions { clut, uv },
        );

        GP0State::AwaitCommand
    }
}
